...
// Apparel partner marks (Simple Icons path data, 24x24 viewBox).
...
/** The partner's mark, top-right of the card. Jordan Brand schools
 *  ("Nike / Jordan") get the Jumpman, not the swoosh. */
function PartnerMark({ brand }: { brand: string }) {
  const key = brand.includes('Jordan')
    ? 'Jordan'
    : Object.keys(BRAND_PATHS).find((k) => brand.startsWith(k));
  if (!key) return null;
  return (
    <svg className="eoy-partner-mark" viewBox="0 0 24 24" aria-label={key}>
      <path fill="currentColor" d={BRAND_PATHS[key]} />
    </svg>
  );
}
